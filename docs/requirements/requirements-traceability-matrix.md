...
| MVP-006 | Visible persistent agentic memory | AC-012 | Trace, database, restart, and demo evidence | In progress; CockroachDB-ready memory boundaries, file-lineage projection contracts, the `file_lineage_events` migration/repository, a lineage record/list command, cockpit-visible lineage preview state for reviewed proposals, and a sanitized live-memory validation command exist locally, but a convincing live CockroachDB memory table view, restart trace, and demo evidence remain pending |
