...
| COMP-006 | At least one AWS service | Amazon Bedrock and Amazon Simple Storage Service, plus approved compute | Runtime traces and architecture evidence | In progress; live AWS foundation uses Amazon S3, Amazon SQS, AWS Lambda, Amazon API Gateway, Amazon CloudWatch Logs, and Amazon Bedrock Runtime; the worker verifies S3 PDF artifacts and has live smoke evidence for bounded Bedrock document classification, while CockroachDB-backed cloud persistence and public demo evidence remain pending |
...
| PROD-008 | Complete desktop and cloud product parity | `product-requirements.md` FR-047–FR-050 | Cross-surface contract and end-to-end tests | In progress; local desktop discovery, cancellation, session state, guarded read-only central cockpit PDF preview, confirmed safe external-link delegation, sanitized fail-closed runtime preflight readiness display with Analyze-time retry, tested 30-PDF Analyze batch dispatch, per-item progress, terminal batch status, proposal-to-review table updates, visible proposed rename/move targets, local proposal review decision controls, offline durable review persistence, cockpit durable review wiring for persisted proposal IDs, live cloud upload and queue boundaries, S3 artifact verification, and bounded Bedrock document-classification worker smoke evidence exist, while live cloud CockroachDB persistence, live desktop analysis execution, and full cloud parity remain pending |
