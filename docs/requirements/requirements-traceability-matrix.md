...
| QUAL-004 | Integration and contract tests | Real CockroachDB and mocked or sandboxed AWS boundaries as appropriate | Integration report | In progress; offline migrations and local operation, classification, and review-decision transaction contracts are complete; revision `0001` has isolated live schema evidence, while revisions `0002` and `0003` and online application integration remain pending live validation |
...
| MVP-003 | Identity-safe move and copy | AC-005, AC-006 | Filesystem execution and persistence integrity tests | In progress; local operation planning, approval binding, restore contracts, mass rename/move previews, and lineage-row projections are tested without mutating files; physical CockroachDB lineage persistence and UI execution controls remain pending |
...
| MVP-006 | Visible persistent agentic memory | AC-012 | Trace, database, restart, and demo evidence | In progress; CockroachDB-ready memory boundaries and file-lineage projection contracts exist locally, but a convincing live CockroachDB memory table view, restart trace, and demo evidence remain pending |
