...
| QUAL-004 | Integration and contract tests | Real CockroachDB and mocked or sandboxed AWS boundaries as appropriate | Integration report | In progress; offline migrations and local operation and classification transaction contracts are complete; revision `0001` has isolated live schema evidence, while revision `0002` and online application integration remain pending live validation |
...
| SEC-014 | Human approval for material actions | Review decision and execution state machine | Approval contract tests and end-to-end tests | In progress; operation-plan approval validation and proposal review-decision contracts are locally tested, with exact proposal and operation-plan fingerprint binding; cockpit approval controls and durable review persistence remain pending |
...
| MVP-002 | Confidence review and quality sampling | AC-003, AC-004 | User-flow tests and calibration evidence | In progress; local proposal review-decision contracts and append-only review history tests exist, while visible user-flow controls, quality sampling, and calibration evidence remain pending |
