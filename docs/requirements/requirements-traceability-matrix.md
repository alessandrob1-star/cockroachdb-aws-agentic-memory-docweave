...
| MVP-006 | Visible persistent agentic memory | AC-012 | Trace, database, restart, and demo evidence | In progress; CockroachDB-ready memory boundaries, file-lineage projection contracts, the `file_lineage_events`, `cloud_analysis_jobs`, and `cloud_analysis_objects` migrations, a lineage record/list command, cockpit-visible lineage preview state for reviewed proposals, a sanitized live-memory validation command, a read-only memory evidence report, and a cloud worker memory writer exist locally, but live table-count evidence for revision `0005`, restart trace, and demo evidence remain pending |
