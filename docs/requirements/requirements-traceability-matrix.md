...
| PROD-003 | Explainable analysis, classification, naming, and relationships | `product-requirements.md` FR-011–FR-017, `domain-data-requirements.md` taxonomy baseline, and `classification-and-confidence-specification.md` | Curated-corpus evaluation and trace evidence | In progress; extraction, `classification.v1`, the pinned Bedrock gateway, one accepted live synthetic classification, atomic proposal persistence, document registration, approved taxonomy initialization, explicit runtime orchestration, side-effect-free application runtime composition, a controlled single-PDF runtime command, desktop Analyze dispatch, visible non-authoritative review state, and versioned uncalibrated review-ordering scores are implemented; calibration, thresholds, live desktop runtime execution, corpus quality, naming, relationships, final review decisions, and evaluation remain pending |
