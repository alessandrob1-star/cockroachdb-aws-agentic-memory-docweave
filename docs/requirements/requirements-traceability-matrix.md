...
| ENV-002 | CockroachDB Cloud cluster | `docweave-memory`, CockroachDB Cloud Basic on AWS Frankfurt with free-resource limits | Verified environment with isolated initial validation schema and side-effect-free runtime engine composition; no live application transaction or production deployment |
...
| ENV-007 | Amazon Bedrock primary profile | Temporary European Amazon Nova 2 Lite profile active in `eu-central-1` | One bounded synthetic classification validated and side-effect-free Bedrock client composition implemented; desktop-triggered runtime invocation and corpus-quality evaluation pending |
...
| PROD-003 | Explainable analysis, classification, naming, and relationships | `product-requirements.md` FR-011–FR-017, `domain-data-requirements.md` taxonomy baseline, and `classification-and-confidence-specification.md` | Curated-corpus evaluation and trace evidence | In progress; extraction, `classification.v1`, the pinned Bedrock gateway, one accepted live synthetic classification, atomic proposal persistence, document registration, approved taxonomy initialization, explicit runtime orchestration, side-effect-free application runtime composition, and versioned uncalibrated review-ordering scores are implemented; calibration, thresholds, desktop-triggered invocation, corpus quality, naming, relationships, review, and evaluation remain pending |
