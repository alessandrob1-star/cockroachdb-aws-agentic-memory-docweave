...
| MVP-005 | Batch restore and accountability | AC-010, AC-011 | Role, audit, and partial-failure tests | In progress; local batch accountability, partial-failure tests, per-item restore blockers, individual restore execution, bounded batch restore orchestration, explicit restore audit events, CockroachDB-ready audit append evidence, and a durable restore audit recorder boundary are implemented, while roles and full durable restore workflow remain pending |
