...
| PROD-008 | Complete desktop and cloud product parity | `product-requirements.md` FR-047–FR-050 | Cross-surface contract and end-to-end tests | In progress; local desktop discovery, cancellation, session state, guarded read-only central cockpit PDF preview, confirmed safe external-link delegation, sanitized fail-closed runtime preflight readiness display with Analyze-time retry, tested 30-PDF Analyze batch dispatch, per-item progress, terminal batch status, proposal-to-review table updates, visible proposed rename/move targets, local proposal review decision controls, offline durable review persistence, and cockpit durable review wiring for persisted proposal IDs exist, while live desktop analysis execution and cloud parity remain pending |
