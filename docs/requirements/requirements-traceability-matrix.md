...
| PROD-005 | Safe copy, move, resume, and restore | `product-requirements.md` FR-025–FR-034 | Planning, execution, failure-injection, and end-to-end tests | In progress; durable intent/result orchestration, restart-state handling, classification-derived non-mutating copy planning, review-decision binding to the exact operation-plan preview, local restore-preview and approved restore-execution contracts for successful copy and move results, bounded local batch restore orchestration, local restore audit events, CockroachDB-ready restore audit mapping/append tests, a durable restore audit recorder boundary, a bounded restore audit history reader exposed by runtime and application-runtime composition, cockpit read-only restore-history readiness, and cockpit confirmation that review decisions do not execute copy or move operations are locally tested; live persistence, cockpit execution, rendered restore history rows, UI restore controls, and end-to-end durable restore workflow remain pending |
