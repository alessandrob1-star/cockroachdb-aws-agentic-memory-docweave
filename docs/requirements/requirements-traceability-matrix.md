...
| MVP-006 | Visible persistent agentic memory | AC-012 | Trace, database, restart, and demo evidence | In progress; CockroachDB-ready memory boundaries now include the judge-facing `docweave_judged` schema with `documents`, `agent_runs`, `proposals`, `human_decisions`, `file_history`, and `document_relationships`, plus internal file-lineage and cloud-analysis tables. Live table-count evidence for revision `0007`, runtime writes into the judged schema, restart trace, and demo evidence remain pending |
