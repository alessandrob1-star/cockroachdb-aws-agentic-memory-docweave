...
| ENV-007 | Amazon Bedrock primary profile | Temporary European Amazon Nova 2 Lite profile active in `eu-central-1` | One bounded synthetic classification validated; application wiring and corpus-quality evaluation pending |
...
| PROD-007 | Four persistent memory classes in CockroachDB | `product-requirements.md` FR-041–FR-046 | Schema, retrieval, authorization, and demo evidence | In progress; operational, episodic agent-run, taxonomy, proposal, and evidence write boundaries exist locally, while configured live runtime, retrieval, semantic vectors, preferences, authorization, and demo evidence remain pending |
| PROD-008 | Complete desktop and cloud product parity | `product-requirements.md` FR-047–FR-050 | Cross-surface contract and end-to-end tests | In progress; local desktop discovery, cancellation, session state, guarded read-only central cockpit PDF preview, and confirmed safe external-link delegation exist, while complete review, desktop workflow, and cloud parity remain pending |
