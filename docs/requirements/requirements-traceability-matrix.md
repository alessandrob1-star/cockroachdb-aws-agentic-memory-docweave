...
| SEC-014 | Human approval for material actions | Review decision and execution state machine | Approval contract tests and end-to-end tests | In progress; operation-plan approval validation, proposal review-decision contracts, cockpit approval controls, and offline durable review persistence are locally tested with exact proposal and optional operation-plan fingerprint binding; CockroachDB live validation and desktop durable wiring remain pending |
...
| PROD-004 | Confidence-driven and sampled human review | `product-requirements.md` FR-018–FR-024 | User-flow tests and calibration report | In progress; uncalibrated review-ordering scores, local append-only proposal review-decision contracts, cockpit APPROVE/REJECT controls for selected review rows, and offline CockroachDB review-decision persistence exist, while sortable review queue controls, sampling, calibration, live validation, and desktop durable wiring remain pending |
...
| PROD-008 | Complete desktop and cloud product parity | `product-requirements.md` FR-047–FR-050 | Cross-surface contract and end-to-end tests | In progress; local desktop discovery, cancellation, session state, guarded read-only central cockpit PDF preview, confirmed safe external-link delegation, sanitized fail-closed runtime preflight readiness display, tested 30-PDF Analyze batch dispatch, per-item progress, terminal batch status, proposal-to-review table updates, visible proposed copy targets, local proposal review decision controls, and offline durable review persistence exist, while desktop durable review wiring, live desktop analysis execution, and cloud parity remain pending |
