...
| QUAL-004 | Integration and contract tests | Real CockroachDB and mocked or sandboxed AWS boundaries as appropriate | Integration report | In progress; offline migrations and local operation, classification, review-decision, and file-lineage transaction contracts are complete; the file-lineage command-line runtime can record and list CockroachDB lineage rows when a migrated target is supplied; revision `0001` has isolated live schema evidence, while revisions `0002`, `0003`, and `0004` and online application integration remain pending live validation |
