...
| PROD-004 | Confidence-driven and sampled human review | `product-requirements.md` FR-018–FR-024 | User-flow tests and calibration report | In progress; uncalibrated review-ordering scores and local append-only proposal review-decision contracts exist, while sortable review queue controls, sampling, calibration, and durable human outcomes remain pending |
| PROD-005 | Safe copy, move, resume, and restore | `product-requirements.md` FR-025–FR-034 | Planning, execution, failure-injection, and end-to-end tests | In progress; durable intent/result orchestration, restart-state handling, classification-derived non-mutating copy planning, and review-decision binding to the exact operation-plan preview are locally tested; live persistence, cockpit execution, and restore remain pending |
...
| PROD-008 | Complete desktop and cloud product parity | `product-requirements.md` FR-047–FR-050 | Cross-surface contract and end-to-end tests | In progress; local desktop discovery, cancellation, session state, guarded read-only central cockpit PDF preview, confirmed safe external-link delegation, sanitized fail-closed runtime preflight readiness display, tested 30-PDF Analyze batch dispatch, per-item progress, terminal batch status, proposal-to-review table updates, and visible proposed copy targets exist, while complete review, live desktop analysis execution, and cloud parity remain pending |
