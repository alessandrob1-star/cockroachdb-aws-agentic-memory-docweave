...
| QUAL-004 | Integration and contract tests | Real CockroachDB and mocked or sandboxed AWS boundaries as appropriate | Integration report | In progress; offline migrations and local operation, classification, review-decision, and file-lineage transaction contracts are complete; a packaged live-memory validation command now records sanitized offline evidence and can explicitly inspect or upgrade a configured CockroachDB target; the file-lineage command-line runtime can record and list CockroachDB lineage rows when a migrated target is supplied; revision `0001` has isolated live schema evidence, while revisions `0002`, `0003`, and `0004` and online application integration remain pending live validation |
...
| PROD-008 | Complete desktop and cloud product parity | `product-requirements.md` FR-047–FR-050 | Cross-surface contract and end-to-end tests | In progress; local desktop discovery, cancellation, session state, guarded read-only central cockpit PDF preview, confirmed safe external-link delegation, sanitized fail-closed runtime preflight readiness display with Analyze-time retry, tested 30-PDF Analyze batch dispatch, per-item progress, terminal batch status, proposal-to-review table updates, visible proposed rename/move targets, local proposal review decision controls, offline durable review persistence, cockpit durable review wiring for persisted proposal IDs, live cloud upload and queue boundaries, S3 artifact verification, bounded Bedrock document-classification worker smoke evidence, and cloud analysis result artifact retrieval exist, while live cloud CockroachDB persistence, live desktop analysis execution, and full cloud parity remain pending |
...
| MVP-006 | Visible persistent agentic memory | AC-012 | Trace, database, restart, and demo evidence | In progress; CockroachDB-ready memory boundaries, file-lineage projection contracts, the `file_lineage_events` migration/repository, a lineage record/list command, cockpit-visible lineage preview state for reviewed proposals, a sanitized live-memory validation command, and a read-only memory evidence report exist locally, but live table-count evidence, restart trace, and demo evidence remain pending |
